...
用法 (usage)：
    ...
    python test_api.py --base https://your-endpoint/v1 --key sk-你的key \
        --model gpt-image-2 --prompt "把这些拼成一张海报" \
...
def main():
    ...
    ap.add_argument("--model", default="gpt-image-2", help="模型名，例如 gpt-image-2")
    ...
    common = {"model": args.model, "prompt": args.prompt, "n": args.n}
    if args.size:
        common["size"] = args.size
    for k, v in (("quality", args.quality), ("background", args.background),
    ...
    if args.image:
        ...
        r = requests.post(base + "/images/generations", json=common,
                          headers=headers, timeout=args.timeout)
